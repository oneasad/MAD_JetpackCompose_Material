package com.oneasad.emailloginjetpackcompose.myviewmodels
...
import com.oneasad.emailloginjetpackcompose.models.User
...
class SignInViewModel : ViewModel() {
    ...
    val user = MutableLiveData<com.oneasad.emailloginjetpackcompose.models.User?>(null)
                ...
                result.fold( // It allows you to specify actions for both success and failure cases of the operation, making it easy to manage the different outcomes.
                    onSuccess = { authResult ->
                        ...
                        if (currentUser != null) {
                            ...
                            navController.navigate(ProfileEnum.profile.name)
                        }
                    },
                    onFailure = { e ->
    ...
    // Function to handle Email Sign-In and Sign-Up
    fun handleEmailAuth(email: String, password: String, isSignUp: Boolean, context: Context, navController: NavController) {
        viewModelScope.launch {
            if (isSignUp) {
                emailSignUp(email, password, context, navController).collect { result ->
                    result.fold(
                        onSuccess = { authResult ->
                            val currentUser = authResult.user
                            if (currentUser != null) {
                                user.value = User(currentUser.uid, currentUser.displayName ?: "Muhammad Asad Bashir", "", currentUser.email ?: "", "Pakistan", "Free")
                ...
                emailSignIn(email, password, context, navController).collect { result ->
                    result.fold(
                        onSuccess = { authResult ->
                            val currentUser = authResult.user
//                            val userString = "${currentUser?.uid} ${currentUser?.displayName ?: "No_name"} ${currentUser?.email}"
//                            Log.d("Asad", "handleEmailAuth: " + userString)
                            if (currentUser != null) {
                                user.value = User(currentUser.uid, currentUser.displayName ?: "Muhammad Asad Bashir", "https://picsum.photos/200", currentUser.email ?: "", "Pakistan", "Free")
    ...
    private suspend fun emailSignIn(email: String, password: String, context: Context, navController: NavController): Flow<Result<AuthResult>> {
        val firebaseAuth = FirebaseAuth.getInstance()
    ...
    private suspend fun emailSignUp(email: String, password: String, context: Context, navController: NavController): Flow<Result<AuthResult>> {
        val firebaseAuth = FirebaseAuth.getInstance()
    ...
    fun signOut(navController: NavController) {
        viewModelScope.launch {
            ...
            // Clear the user data in LiveData
            user.value = null

            // Navigate back to the login screen
            navController.navigate(ProfileEnum.home.name)
        }
    }
